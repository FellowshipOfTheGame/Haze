...
typedef struct
{
	...
	dr_uint16 bytesPerSample;

    // Equal to fmt.formatTag, or the value specified by fmt.subFormat if fmt.formatTag is equal to 65534 (WAVE_FORMAT_EXTENSIBLE).
    dr_uint16 translatedFormatTag;
...
dr_int16* drwav_open_and_read_s16(drwav_read_proc onRead, drwav_seek_proc onSeek, void* pUserData, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount);
float* drwav_open_and_read_f32(drwav_read_proc onRead, drwav_seek_proc onSeek, void* pUserData, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount);
dr_int32* drwav_open_and_read_s32(drwav_read_proc onRead, drwav_seek_proc onSeek, void* pUserData, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount);
#ifndef DR_WAV_NO_STDIO
// Opens an decodes a wav file in a single operation.
dr_int16* drwav_open_and_read_file_s16(const char* filename, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount);
float* drwav_open_and_read_file_f32(const char* filename, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount);
dr_int32* drwav_open_and_read_file_s32(const char* filename, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount);
#endif

// Opens an decodes a wav file from a block of memory in a single operation.
dr_int16* drwav_open_and_read_memory_s16(const void* data, size_t dataSize, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount);
float* drwav_open_and_read_memory_f32(const void* data, size_t dataSize, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount);
dr_int32* drwav_open_and_read_memory_s32(const void* data, size_t dataSize, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount);
#endif
...
static dr_uint32 drwav__is_little_endian()
{
    int n = 1;
    return (*(char*)&n) == 1;
}

static dr_uint16 drwav__bytes_to_u16(const unsigned char* data)
{
...
static dr_uint32 drwav__bytes_to_u32(const unsigned char* data)
{
    if (drwav__is_little_endian()) {
...
dr_bool32 drwav_init(drwav* pWav, drwav_read_proc onRead, drwav_seek_proc onSeek, void* pUserData)
{
    ...
    if (pWav->container == drwav_container_riff) {
		...
		dr_uint32 chunkSize = drwav__bytes_to_u32(chunkSizeBytes);
        if (chunkSize < 36) {
	...
	dr_uint16 translatedFormatTag = fmt.formatTag;
    ...
    pWav->bytesPerSample      = (dr_uint32)(fmt.blockAlign / fmt.channels);
...
static dr_uint16 g_drwavAlawTable[256] = {
...
static dr_uint16 g_drwavMulawTable[256] = {
...
static int drwav__pcm_to_f32(float* pOut, const unsigned char* pIn, size_t sampleCount, dr_uint16 bytesPerSample)
{
    ...
    {
        dr_uint32 sample = 0;
        dr_uint32 shift  = (8 - bytesPerSample) * 8;
        for (dr_uint16 j = 0; j < bytesPerSample && j < 4; ++j) {
            sample |= (dr_uint32)(pIn[j]) << shift;
            shift  += 8;
        }
...
static int drwav__ieee_to_f32(float* pOut, const unsigned char* pIn, size_t sampleCount, dr_uint16 bytesPerSample)
{
    if (pOut == NULL || pIn == NULL) {
        return 0;
    }
...
void drwav_s24_to_f32(float* pOut, const dr_uint8* pIn, size_t sampleCount)
{
    ...
    for (size_t i = 0; i < sampleCount; ++i) {
		...
		dr_uint32 sample32 = (dr_uint32)((s0 << 8) | (s1 << 16) | (s2 << 24));
...
static int drwav__pcm_to_s32(dr_int32* pOut, const unsigned char* pIn, size_t totalSampleCount, dr_uint16 bytesPerSample)
{
    if (pOut == NULL || pIn == NULL) {
        return 0;
    }
...
static int drwav__ieee_to_s32(dr_int32* pOut, const unsigned char* pIn, size_t totalSampleCount, dr_uint16 bytesPerSample)
{
    if (pOut == NULL || pIn == NULL) {
        return 0;
    }
...
void drwav_s24_to_s32(dr_int32* pOut, const dr_uint8* pIn, size_t sampleCount)
{
    ...
    for (size_t i = 0; i < sampleCount; ++i) {
        dr_uint32 s0 = pIn[i*3 + 0];
        dr_uint32 s1 = pIn[i*3 + 1];
        dr_uint32 s2 = pIn[i*3 + 2];
...
dr_int16* drwav__read_and_close_s16(drwav* pWav, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount)
{
    assert(pWav != NULL);
...
float* drwav__read_and_close_f32(drwav* pWav, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount)
{
    assert(pWav != NULL);
...
dr_int32* drwav__read_and_close_s32(drwav* pWav, dr_uint16* channels, dr_uint32* sampleRate, dr_uint64* totalSampleCount)
{
    assert(pWav != NULL);
